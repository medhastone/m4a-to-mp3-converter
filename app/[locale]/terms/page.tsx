...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: "Terms - M4A to MP3 Converter",
...
export default function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="flex-1 relative z-10 w-full max-w-4xl mx-auto px-6 py-12 md:py-20 flex flex-col gap-10">
      ...
      {/* Section 6 */}
      <section className="bg-surface-container-high/80 border border-emerald-500/20 shadow-[0_0_20px_rgba(249,115,22,0.05)] rounded-2xl p-8 mt-4 relative overflow-hidden">
        <div className="absolute -right-16 -top-16 w-40 h-40 bg-emerald-500/10 rounded-full blur-[40px]" />
        <h2 className="text-xl font-bold text-on-surface mb-4">
          {t('section6_title')}
        </h2>
        <p className="text-on-surface-variant leading-relaxed mb-4 uppercase text-sm tracking-wide">
          {t('section6_p1')}
        </p>
        <p className="text-on-surface-variant leading-relaxed">
          {t('section6_p2')}
